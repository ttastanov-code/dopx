...
на стороне источника данных — KFF или Sportmonks, в зависимости от того,
чем матч был синкнут — или сбой при импорте, не в текущей view-логике).
    ...
    def handle(self, *args, **options):
        ...
        if suspicious == 0:
            ...
            self.stdout.write(self.style.ERROR(
                ...
                f'на каждый матч, а не реальную заявку. Нужно проверить сырой ответ Sportmonks API '
                f'(lineups include на fixture) для нескольких матчей этой команды и, если источник '
                f'действительно присылает полный состав вместо протокола — перепарсить составы '
                f'после уточнения фильтрации в parsers/sportmonks/importers.py::import_lineups.'
            ))
        else:
