...
class ContactSubmission(BaseModel):
    """Обращение пользователя через форму на сайте"""
    ...
    guest_email = models.EmailField(
        _('Email гостя'),
        blank=True,
        help_text=_('Заполняется если пользователь не авторизован')
    )
    ...
    attachment = models.FileField(
        ...
        null=True,
        blank=True,
        help_text=_('Скриншот или документ (макс. 5MB)')
    )
    ...
    admin_response = models.TextField(
        _('Ответ админа'),
        blank=True,
        help_text=_('Внутренний ответ для истории')
    )
    ...
    def contact_email(self):
        """Возвращает email пользователя или гостя"""
