...
class ContactSubmission(BaseModel):
    ...
    CATEGORY_CHOICES = [
        ...
        # НОВОЕ (продуктовый аудит, раздел 4 "Юридический риск" — право на
        # ответ): отдельная категория для игроков/тренеров/судей/клубов,
        # оспаривающих свой рейтинг или данные о себе. Не переиспользуем
        # 'evaluation' (это про баг в МЕХАНИКЕ оценки, а не про содержание
        # конкретного рейтинга) — юридически значимые обращения должны
        # быть отличимы от обычных багрепортов на уровне фильтра в админке,
        # а не угадываться модератором по тексту сообщения.
