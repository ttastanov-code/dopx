...
"""Отправляет по образцу каждого письма DOPX для визуальной проверки.
Получатель по умолчанию — email пользователя admin (или первого суперпользователя).
Объекты не сохраняются в БД. Отправка через реальный EMAIL_BACKEND, HTML + текстовая версия.
"""
...
class Command(BaseCommand):
    help = (
        "Отправляет по одному образцу каждого письма DOPX "
    ...
    def handle(self, *args, **options):
        ...
        # Нет admin — несохранённый User для {{ user.username }}.
