...
RACE CONDITION В `update_match_statuses` — АНАЛИЗ И ФИКС
----------------------------------------------------------
Корневая причина найдена в `dopx/settings.py::CELERY_BEAT_SCHEDULE`: одна и
та же задача `parsers.tasks.update_match_statuses` зарегистрирована в
расписании ДВАЖДЫ под разными именами:

    'update-live-matches':      crontab(minute='*/2')
    'update-scheduled-matches': crontab(minute='*/10')

Обе записи запускают ОДНУ И ТУ ЖЕ функцию без каких-либо различающих
параметров, а сама функция внутри всегда выбирает ВСЕ матчи со статусом
`scheduled`/`live` (`Match.objects.filter(status__in=['scheduled', 'live'])`)
— то есть оба расписания синхронизируют один и тот же набор матчей. На
каждой отметке, кратной 10 минутам (:00, :10, :20 ...), Celery Beat
поставит в очередь ДВЕ параллельные задачи почти одновременно. При наличии
более одного воркера (что обязательно для прод-инсталляции) они запустятся
конкурентно.

Внутри `import_events_and_minutes` (parsers/kff/importers.py) выполняется
`match.events.all().delete()` с последующим bulk-созданием новых событий,
обёрнутое в `@transaction.atomic`. `transaction.atomic()` гарантирует
атомарность ОДНОЙ транзакции, но НЕ мешает ВТОРОЙ параллельной транзакции
делать то же самое над теми же строками — на уровне PostgreSQL это приводит
к одному из трёх сценариев:
  1. Обе транзакции одновременно проходят DELETE, обе создают события заново
     — в лучшем случае двойная работа, в худшем — временное отсутствие
     событий в момент между DELETE одной транзакции и COMMIT другой.
  2. Одна транзакция блокируется на DELETE, ожидая снятия блокировки другой
     — рост времени отклика задачи вплоть до срабатывания таймаута воркера.
  3. Deadlock, если порядок блокировок строк отличается (например, из-за
     разного порядка событий, вернувшихся от API) — Postgres принудительно
     прерывает одну из транзакций с ошибкой `deadlock detected`.

ФИКС (два независимых уровня защиты):
  A. Redis-дистрибьютед-лок НА КАЖДЫЙ МАТЧ (не на всю задачу целиком —
     иначе воркер B не смог бы синхронизировать вообще ни один матч, пока
     воркер A обрабатывает СВОЙ список; блокировка на уровне матча
     позволяет воркерам параллельно, но безопасно, обрабатывать РАЗНЫЕ
     матчи). Лок берётся атомарно через `cache.add()` (аналог Redis
     `SETNX`) ДО любых сетевых вызовов к внешнему API — если воркер B не
     смог взять лок, он логирует пропуск и переходит к следующему матчу,
     не поднимая исключений.
  B. `select_for_update(nowait=True)` — построчная блокировка Postgres,
     срабатывающая непосредственно в момент записи (defense-in-depth: даже
     если Redis-лок по какой-то причине не сработал — например, кэш был
     эвакуирован — конкурентная запись всё равно будет остановлена на
     уровне БД). `nowait=True` принципиален: без него второй воркер просто
     встал бы в очередь ожидания блокировки на неопределённое время; с ним
     Postgres немедленно возвращает ошибку `OperationalError`, которую мы
     ловим и превращаем в контролируемый skip.

Важно: Redis-лок держится на всё время обращения к внешнему API (сетевой
I/O), а `select_for_update` — только на короткий момент самой записи в БД.
Если бы было наоборот (блокировка строки на время HTTP-запроса к KFF API),
это держало бы открытую транзакцию Postgres на неопределённое время сетевого
ожидания — прямой путь к исчерпанию пула соединений БД при деградации
внешнего API.
"""
...
def update_match_statuses(self):
    ...
    for match in active_matches:
        ...
        try:
            ...
            # ПОПУТНЫЙ ФИКС N+1: раньше на КАЖДОЕ событие из ответа API
            # выполнялся отдельный `MatchEvent.objects.filter(...).exists()`
            # запрос (плюс вычисленный `existing_event_ids` вообще не
            # использовался дальше — мёртвый код). Теперь существующие
            # события матча читаются ОДНИМ запросом и группируются в памяти
            # по минуте; семантика `icontains` из оригинала сохранена через
            # проверку вхождения подстроки.
