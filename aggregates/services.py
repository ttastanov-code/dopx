...
# числом рейтинг уже показывается как число, но помечается "Оценок хватает", а
# не "Оценок много" (метки переименованы 2026-09-10, см. _TIER_META в
# core/templatetags/rating_extras.py) — 5 голосов статистически достаточно,
# чтобы не считаться шумом одного тролля, но недостаточно, чтобы считаться
# устоявшимся консенсусом. Используется только для UI-бейджа
# (core/templatetags/rating_extras.py::confidence_badge), НЕ влияет на то,
# показывается ли число вообще — за это по-прежнему отвечает
