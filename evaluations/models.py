...
class EvaluationSession(BaseModel):
    ...
    # кроме стартовых игроков) и другими виджетами на шагах "Тренеры"/
    # "Судья" (три крупные кнопки вместо отдельных шкал на каждый
    # под-критерий). 'full' — сегодняшнее поведение по умолчанию: и для уже
    # существующих сессий (миграция не трогает старые строки), и когда
    # пользователь явно выбрал "Подробно" на шаге 1.
    MODE_CHOICES = [
        ('full', _('Подробно')),
        ('quick', _('Быстро')),
    ]
    mode = models.CharField(_('Режим оценки'), max_length=10, choices=MODE_CHOICES, default='full')
