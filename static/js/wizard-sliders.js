...
(function () {
    ...
    //
    // БАГ, КОТОРЫЙ ТУТ БЫЛ (найден пользователем на "Оценке судейства",
    // 2026-09-07): на шаге "Судья" оба слайдера двигались ОДНИМ и тем же
    // процентом в ОДНУ сторону, хотя по смыслу они противоположны —
    // "Влияние на матч" 0=незаметен/хорошо, 100=решил исход/плохо, а
    // "Качество решений" 1=плохо, 10=отлично. Кнопка "Слабо повлиял"
    // (pct=15, т.е. хороший, незаметный судья) тянула decision_quality
    // тоже к низу шкалы (~2 из 10 — "плохое качество"), хотя незаметный
    // судья обычно означает как раз ХОРОШЕЕ качество решений. Слайдер с
    // атрибутом data-wz-invert="true" теперь получает (100 - pct) вместо
    // pct — единственное такое место сейчас decision_quality в
    // _referee_criteria.html; остальные шаги (Тренеры/Матч), где "выше
    // всегда лучше" для всех критериев разом, атрибут не ставят и ведут
    // себя как раньше.
    //
