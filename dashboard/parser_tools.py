...
# ИСПРАВЛЕНО (баг: "Кайрат" русской К не находит "Қайрат" с казахской Қ):
# 9 казахских букв кириллицы (Ә Ғ Қ Ң Ө Ұ Ү Һ І) визуально похожи на
# русские, но это ДРУГИЕ Unicode-символы — SQL icontains сравнивает байты,
# для него "Қ" (U+049A) и "К" (U+041A) совершенно не связаны. У staff
# часто нет казахской раскладки клавиатуры, и он печатает ближайшую
# русскую букву — раньше это означало "поиск ничего не найдёт", хотя
# команда с точным нужным именем в базе есть. Схлопываем обе буквы в
# один канонический символ ПЕРЕД сравнением.
...
def search_matches(query: str, year: int | None = None) -> dict:
    """Поиск по названию домашней/гостевой команды — самый частый сценарий
    staff: "видел проблему у матча КАЙРАТ вчера, какой у него UUID?".
    Отдельно: если запрос — чистое число, считаем это external_id (KFF
    game id) и ищем ТОЧНОЕ совпадение по ВСЕЙ базе без ограничения по
    году — это точный идентификатор конкретного матча, сужать его годом
    незачем и вредно (staff мог перепутать/не знать год матча).

    ИСПРАВЛЕНО (баг №1: поиск "Қайрат" не находил реально существующий
    матч "Қайрат — Астана" от 05.04.2026): раньше поиск по названию
    команды резался произвольным числом `limit=30` самых свежих матчей —
    у команды с активной историей нужный, но более старый матч тихо
    выпадал за пределы среза, без единого намёка, что список обрезан.
    Вместо произвольного числа — естественная граница "текущий сезон":
    матчи фильтруются по календарному ГОДУ начала (`start_time__year`,
    по умолчанию — текущий год; тот же год, что в `Season.year`, см.
    seasons/models.py — сезоны КПЛ и так совпадают с календарным годом).
    Внутри одного года у команды физически не может быть сотен матчей
    (двухкруговой турнир на ~14 команд — это максимум пара десятков),
    поэтому здесь больше не нужен и произвольный числовой лимит вообще.
    Если нужного матча всё равно нет — форма даёт выбрать другой год.

    Дополнительно, многословный запрос ("Қайрат Астана") требует
    совпадения КАЖДОГО слова хотя бы с одной из команд — резко сужает
    выдачу до конкретного матча, если staff знает обе команды.

    ИСПРАВЛЕНО (баг №2: "Кайрат" русскими буквами не находит "Қайрат") —
    см. _normalize_kz() выше. Матчинг команд по токенам запроса идёт в
    Python по нормализованному имени (команд в базе — десятки, не тысячи,
    фильтрация в памяти дешевле, чем городить SQL TRANSLATE() или тянуть
    Postgres-расширение unaccent, которое эту пару букв всё равно не
    решает — это не диакритика, а отдельные кириллические коды)."""
