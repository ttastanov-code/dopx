...
TRIGGERABLE_TASKS = {
    ...
    # НОВОЕ (2026-08-31): раньше это была management-команда без кнопки в
    # UI (sync_kff_player_meta, бывшая scrape_kff_photos) — теперь ещё и
    # автоматически снимает is_active игрокам, которых несколько прогонов
    # подряд нет в актуальном составе на kffleague.kz (см. docstring
    # parsers/kff/photo_scraper.py::match_and_fetch_players_for_team).
    # Кнопка нужна, чтобы staff мог прогнать это СРАЗУ (например сразу
    # после жалобы "в составе команды висит игрок, который уже ушёл"),
    # не дожидаясь расписания раз в 3 дня.
    "sync_kff_player_meta": "Синхронизировать ID/позиции игроков и актуальность состава с kffleague.kz",
...
TASK_DESCRIPTIONS: dict[str, str] = {
    ...
    "sync_kff_player_meta": (
        "Сопоставляет игроков с публичным сайтом kffleague.kz (не с JSON "
        "API): проставляет kff_website_id, бэкафиллит пустую позицию, и "
        "проверяет, кто пропал из актуального состава команды на сайте — "
        "если игрока НЕ находят 2 прогона подряд, автоматически снимает "
        "is_active, и он пропадает со страницы команды. Фото НЕ скачивает "
        "(отключено 2026-08-21). Безопасно жать несколько раз подряд — если "
        "нужно СРАЗУ подтвердить, что игрок ушёл, а не ждать расписание раз "
        "в 3 дня, нажмите дважды: второй прогон, если сайт оба раза "
        "подтвердит отсутствие, сразу деактивирует игрока."
    ),
