...
def current_round_squad(request):
    ...
    Номер берётся из round_squad/services.py::resolve_current_tour — той
    же общей функции, которую использует и страница /round/ без явного
    номера тура в URL (round_squad/views.py::_resolve_latest_tour). До
    2026-08-31 у этих двух мест были РАЗНЫЕ приоритеты (см. докстринг
    resolve_current_tour про конкретный баг, пойманный на реальном
    календаре с переносом целого тура) — кнопка в шапке могла показывать
    один тур, а страница /round/ без номера — уже другой. Теперь оба места
    отвечают на вопрос "какой тур сейчас" одинаково.

    Ссылка в навбаре ведёт на этот тур ЯВНО (через season_id/tour в URL),
    а не на round_squad:round без параметров — иначе номер в кнопке и тур,
    который реально откроется по клику, могли бы разойтись.
...
def indicator_tooltips(request):
    ...
    context = {
        'INDICATOR_TOOLTIPS': {
            # === ИГРОКИ ===
            'player': {
                # 2026-09-24: независимая оценка игры по данным матча (matches/stat_ratings.py).
