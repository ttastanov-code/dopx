...
# Второе семейство шрифтов — ТОЛЬКО для build_badge_share_card (2026-09-01,
# второй раунд правок после фидбэка "шрифты уебанские, дай супер-премиум").
# Liberation Sans остаётся дефолтом для остальных 4 функций модуля (og:image
# превью — там его метрическая близость к Arial важнее стиля), но на
# портретной карточке достижения он уже выглядел плоско рядом с фотореальным
# AI-фоном.
#
# Изначально пробовали Poppins (более "премиальный", geometric sans) —
# найден локально в песочнице (`/usr/share/fonts/truetype/google-fonts/`), но
# ОКАЗАЛСЯ БЕЗ КИРИЛЛИЦЫ: наивная проверка `font.getmask(ch).getbbox() is not
# None` ЛОЖНО положительна на "тофу"-глифы (Pillow рисует пустой квадрат-
# заглушку для отсутствующих символов, у которого тоже есть непустой bbox).
# Надёжная проверка — через `fontTools.ttLib.TTFont(path).getBestCmap()` и
# `ord(ch) in cmap`; ей Poppins не прошёл, DejaVu Sans — прошёл на всех нужных
# начертаниях. Poppins-*.ttf удалены из репозитория.
#
# DejaVu — не хуже стилистически: гуманистический sans с более уверенными,
# "весомыми" засечками штрихов, чем у Liberation, плюс есть Condensed-начертание
# (используется ниже для капса — трекинг на узком шрифте даёт editorial-вид).
_BADGE_FONT_FILES = {
...
# --- Второй раунд правок build_badge_share_card (2026-09-01) ---------------
# Фидбэк пользователя по первой AI-фон-версии: "текст нечёткий, белые фоны по
# углам, шрифты уебанские". Три конкретные причины и их фиксы:
#
# 1. "Белые фоны по углам" — AI-генератор кладёт яркий контент (грани
...
#    грубый белесый огрызок. Фикс — `_edge_vignette`: затемняем ВСЕ 4 края
#    независимо от их содержимого, ДО наложения текста, одинаково для любого
#    фона (кастомного AI или процедурного).
#
# 2. "Текст нечёткий" — заголовок/описание накладывались прямо на фотореальный
#    фон без разделяющего слоя; на светлых участках (легендарный кристалл,
#    платиновый лёд) белый текст читался плохо независимо от размера/шрифта.
#    Фикс — `_legibility_scrim`: точный alpha-градиент слева направо
#    (непрозрачно-чёрный у левого края, где стоит весь текст, прозрачный
#    справа, где кристалл/арт) + `_shadow_text`/`_shadow_tracked_text` —
#    у каждой строки текста есть смещённая тёмная "подложка" под ней,
#    гарантирующая контраст независимо от того, что именно под ней на фоне.
#
# 3. "Шрифты уебанские" — Liberation Sans не имеет ни одного акцентного
#    начертания уже, поэтому капс-надписи выглядели как обычный текст с
#    Caps Lock, а не типографский приём. Фикс — переход на DejaVu Sans
#    (см. докстринг `_badge_font`), включая Condensed-начертание для
#    трекнутого капса (бренд/пилюля/эпиграф/футер) — тот самый "editorial"
#    приём премиальных карточек.
...
def build_badge_share_card(
    *, username: str, badge_code: str, badge_name: str, badge_description: str,
    ...
    # Второй раунд правок (2026-09-01) — применяются ОДИНАКОВО к кастомному
    # AI-фону и к процедурному фолбэку, ДО любого текста:
    #  1. `_edge_vignette` — гасит яркий контент у самой рамки (фикс
    #     "белых пятен по углам" после скругления).
    #  2. `_legibility_scrim` — тёмный горизонтальный градиент под всей
    #     текстовой (левой) зоной, не трогающий иллюстрацию справа.
    # См. докстринг блока над `_edge_vignette` — там разбор каждой причины.
    # strength поднята с 0.65 до 0.85 (2026-09-01, повторный фидбэк "белые
    # фоны по углам" на реальном проде) — на 65% часть исходной яркости
    # AI-фона всё ещё пробивалась через притемнение у самого края, и в
    # скруглённом вырезе (`_rounded_alpha_mask`, radius=36) это читалось как
    # грубое светлое пятно ровно там, где угол уже должен быть почти чёрным.
