...
# 2026-09-09 (жалоба пользователя со скриншотом: "из-за бейджей этих ломаются
# таблицы в разных страницах сайта") — КОРНЕВАЯ ПРИЧИНА: сам бейдж —
# whitespace-nowrap (components/_confidence_badge.html), а самая длинная
# метка "Высокая надёжность" (19 символов) — это ровно тот тир, что чаще
# всего показывается (сложившиеся рейтинги игроков с историей). В таблице
# `table-layout: auto` одна такая ячейка растягивает всю колонку под ширину
# нужную для нерушимого куска текста, даже если по факту он перенёсся на
# вторую строку внутри flex-wrap — соседние колонки (Вклад/Риск/Клатч)
# остаются узкими, и таблица выглядит перекошенной. Та же метка внутри
# flex-колонки (карточка "Лучшие матчи") раздувает высоту строки и создаёт
# пустое пространство при items-center. Короткие метки — самое дешёвое и
# при этом централизованное исправление (один словарь, а не 5+ шаблонов,
# где вызывается {% confidence_badge %}). Полная формулировка остаётся в
# тултипе и на странице объяснения (core:anti_fraud#confidence) — сам
# бейдж всегда был кратким индикатором, а не единственным источником
# смысла. ВАЖНО: templates/core/anti_fraud.html — легенда с теми же тремя
# бейджами, держать в синхроне с этими подписями руками (страница не
# читает этот словарь, у неё статичная вёрстка).
#
# 2026-09-10 (жалоба пользователя: "надо поменять названия... сейчас нихуя
# не понятно что надежно... видишь 7,2 Надёжно, создается впечатление, что
# это типа рейтинг такой") — КОРНЕВАЯ ПРИЧИНА: короткие метки из правки
# 2026-09-09 решили проблему с версткой, но "Надёжно"/"Умеренно" сами по
# себе читаются как оценочное прилагательное К ЧИСЛУ РЕЙТИНГА ("7.2,
# надёжно" ~ "7.2, хорошо"), а не как характеристика размера выборки
# голосов, которой бейдж на самом деле является. Переименовано так, чтобы
# в КАЖДОЙ метке явно было слово "оценок" — тогда бейдж нельзя прочитать
# иначе, чем "оценок мало/хватает/много", и рейтинг с индикатором рядом
# больше не выглядят одним и тем же числом.
_TIER_META = {
